    ...
    stages {
        ...
        stage('Тест пагинации') {
            steps {
                dir('test_pagination') {
                    withPythonEnv('python') {
                        sh "pytest test_paginator.py --url='${params.url}' --schema=${params.schema} --result=${params.result} --alluredir=${params.report} --no-header --no-summary -q --log-cli-level=INFO"
                    }
                }
            }
        }
